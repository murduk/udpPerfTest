        ...
        if (len > 0)
        {
            ...
            if (databuff[0] == 0xFF && databuff[1] == 0xAA && databuff[len - 2] == 0xAA && databuff[len - 1] == 0xFF)
            {
                ...
                for (int j = 0; j < numberOfEffects; j++)
                {
                    ...
                    ESP_LOGI(TAG, "Blue:%d\n", spAction.blue);
                    spAction.red2 = databuff[12 + j * 12];
                    ESP_LOGI(TAG, "Red2:%d\n", spAction.red2);
                    spAction.green2 = databuff[13 + j * 12];
                    ESP_LOGI(TAG, "Green2:%d\n", spAction.green2);
                    spAction.blue2 = databuff[14 + j * 12];
                    ESP_LOGI(TAG, "Blue2:%d\n", spAction.blue2);
                    int delay = 0;
                    delay = databuff[15 + j * 12];
                    delay = delay << 8;
                    delay = delay | databuff[16 + j * 12];
